...
vector<int> constructTempArray(string pattern) {
    ...
    return lps;
    // return pattern.substr(0,lps[pattern.length()-1]); this tells us if there are prefix and suffix of certain length as at the end if it's 0 it means the pattern didn't match till end .so it will be considered 0 size.But if there is a value then it means that there were serially match and it's size.
}
